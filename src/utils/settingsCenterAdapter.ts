...
export async function getOverview(): Promise<any> {
  if (FORCE_MOCK) return getMockSettingsCenter();
  try {
    const raw = await settingsApi.getSettingsCenterOverview();
    return unwrap(raw);
  } catch (e: unknown) {
    if (isClientError(e)) throw e;
    if (isNetworkOrServerError(e)) { showFallbackWarning(); return getMockSettingsCenter(); }
    throw e;
  }
}
...
export async function getAssistants(): Promise<any> {
...
export async function getRuntime(): Promise<any> {
  if (FORCE_MOCK) return MOCK_RUNTIME;
  try {
    const raw = await settingsApi.getSettingsCenterRuntime();
    return unwrap(raw);
  } catch (e: unknown) {
    if (isClientError(e)) throw e;
    if (isNetworkOrServerError(e)) { showFallbackWarning(); return MOCK_RUNTIME; }
    throw e;
  }
}
